...
from app.models.basic import RelationField, init_relation, Basic
...
class Mapper(object):
    ...
    @connect(True)
    async def insert(cls, *, model: Basic, session: AsyncSession = None, log=False, not_begin=False):
        session.add(model)
        await session.flush()
    ...
    async def insert_log(cls, session, user, mode, now, old=None, key=None, changed=None):
        ...
        model = OperationLog(user, mode, "&".join(title), tag, diff_data, key)
        session.add(model)
    ...
    @classmethod
    async def get_field_alias(cls, session, relation: Tuple[RelationField], name, now, old=None):
        """
...
# 设置项目角色映射关系
init_relation(ProjectRole, RelationField(ProjectRole.user_id, (User.id, User.name)),
              RelationField(ProjectRole.project_id, (Project.id, Project.name)),
              RelationField(ProjectRole.project_role, ProjectRoleEnum.name))

init_relation(RedisInfo, RelationField(RedisInfo.env, (Environment.id, Environment.name)))

init_relation(TestPlan, RelationField(TestPlan.env, (Environment.id, Environment.name)),
              RelationField(TestPlan.project_id, (Project.id, Project.name)),
              RelationField(TestPlan.msg_type, TestPlan.get_msg_type),
              RelationField(TestPlan.receiver, (User.id, User.name)))

init_relation(TestCase)

init_relation(TestCaseAsserts, RelationField(TestCaseAsserts.case_id, (TestCase.id, TestCase.name)))


init_relation(GConfig, RelationField(GConfig.env, (Environment.id, Environment.name)))

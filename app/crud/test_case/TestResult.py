...
class TestResultDao(Mapper):
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    result = TestResult(report_id, case_id, case_name, status,
    ...
    @staticmethod
    async def list(report_id: int) -> List[TestResult]:
        try:
            async with async_session() as session:
                sql = select(TestResult, TestCase.directory_id).join(TestCase,
                                                                     TestCase.id == TestResult.case_id). \
                    where(TestResult.report_id == report_id,
                          TestResult.deleted_at == 0).order_by(
                    asc(TestResult.case_id), asc(TestResult.start_at))

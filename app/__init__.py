...
@ats.exception_handler(RequestValidationError)
...
@ats.exception_handler(PermissionException)
...
@ats.exception_handler(AuthException)
...
def init_logging():
    ...
    # 为ats添加一个info log文件，主要记录debug和info级别的日志
    ats_info = os.path.join(Config.LOG_DIR, f"{Config.ATS_INFO}.log")
    # 为ats添加一个error log文件，主要记录warning和error级别的日志
    ats_error = os.path.join(Config.LOG_DIR, f"{Config.ATS_ERROR}.log")
    logger.add(ats_info, enqueue=True, rotation="20 MB", level="DEBUG", filter=make_filter(Config.ATS_INFO))

    logger.add(ats_error, enqueue=True, rotation="10 MB", level="WARNING", filter=make_filter(Config.ATS_ERROR))

    # 配置loguru的日志句柄，sink代表输出的目标
    logger.configure(
        handlers=[
            {"sink": sys.stdout, "level": logging.DEBUG, "format": format_record},
            {"sink": ats_info, "level": logging.INFO, "format": INFO_FORMAT, "filter": make_filter(Config.ATS_INFO)},
            {"sink": ats_error, "level": logging.WARNING, "format": ERROR_FORMAT,
             "filter": make_filter(Config.ATS_ERROR)}
        ]
    )
    return logger
